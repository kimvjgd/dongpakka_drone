...
void sendDroneCommand()
{
  ...
  // String(ddd, HEX) => ddd를 HEX 타입의 문자로 만들어서 출력
...
void checkCRC()
{
  ...
  // payload 배열의 0부터 14까지 0x00의 값으로 채운다는 의미 (초기화 시키겠다.)
  // memset(A, B, C);
  // A는 버퍼 시작 주소, B는 버퍼에 채우고자 하는 값, C는 채우고자 하는 버퍼의 크기
  //
...
unsigned char startDroneControl() // 제어 시작 설정
{
  if (!digitalRead(9)) // 9 pin == LOW
  {
